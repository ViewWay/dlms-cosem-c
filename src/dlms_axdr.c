    ...
    *w = 0;
    ...
    switch (value->type) {
        ...
        case DLMS_DATA_BOOLEAN:
            if (cap < 2) return DLMS_ERROR_BUFFER;
            dst[pos++] = 3;
            dst[pos++] = value->u.boolean_val ? 1 : 0;
            break;

        case DLMS_DATA_INTEGER:
            if (cap < 2) return DLMS_ERROR_BUFFER;
            dst[pos++] = 15;
            dst[pos++] = (uint8_t)value->u.int8_val;
            break;

        case DLMS_DATA_UNSIGNED_INTEGER:
            if (cap < 2) return DLMS_ERROR_BUFFER;
            dst[pos++] = 17;
            dst[pos++] = value->u.uint8_val;
            break;

        case DLMS_DATA_LONG:
            if (cap < 3) return DLMS_ERROR_BUFFER;
            dst[pos++] = 16;
            dst[pos++] = (uint8_t)(value->u.int16_val >> 8);
            dst[pos++] = (uint8_t)(value->u.int16_val & 0xFF);
            break;

        case DLMS_DATA_UNSIGNED_LONG:
            if (cap < 3) return DLMS_ERROR_BUFFER;
            dst[pos++] = 18;
            dst[pos++] = (uint8_t)(value->u.uint16_val >> 8);
            dst[pos++] = (uint8_t)(value->u.uint16_val & 0xFF);
            break;

        case DLMS_DATA_DOUBLE_LONG:
            if (cap < 5) return DLMS_ERROR_BUFFER;
            dst[pos++] = 5;
            dst[pos++] = (uint8_t)(value->u.int32_val >> 24);
            dst[pos++] = (uint8_t)(value->u.int32_val >> 16);
            dst[pos++] = (uint8_t)(value->u.int32_val >> 8);
            dst[pos++] = (uint8_t)(value->u.int32_val & 0xFF);
            break;

        case DLMS_DATA_DOUBLE_LONG_UNSIGNED:
            if (cap < 5) return DLMS_ERROR_BUFFER;
            dst[pos++] = 6;
            dst[pos++] = (uint8_t)(value->u.uint32_val >> 24);
            dst[pos++] = (uint8_t)(value->u.uint32_val >> 16);
            dst[pos++] = (uint8_t)(value->u.uint32_val >> 8);
            dst[pos++] = (uint8_t)(value->u.uint32_val & 0xFF);
            break;
        ...
        case DLMS_DATA_LONG64:
            ...
            dst[pos++] = 20;
            for (int i = 7; i >= 0; i--)
                dst[pos++] = (uint8_t)(value->u.int64_val >> (8 * i));
            break;
        ...
        case DLMS_DATA_UNSIGNED_LONG64:
            ...
            dst[pos++] = 21;
            for (int i = 7; i >= 0; i--)
                dst[pos++] = (uint8_t)(value->u.uint64_val >> (8 * i));
            break;
        ...
        case DLMS_DATA_FLOAT32:
            if (cap < 5) return DLMS_ERROR_BUFFER;
            dst[pos++] = 23;
            memcpy(dst + pos, &value->u.float32_val, 4);
            pos += 4;
            break;
        ...
        case DLMS_DATA_FLOAT64:
            if (cap < 9) return DLMS_ERROR_BUFFER;
            dst[pos++] = 24;
            memcpy(dst + pos, &value->u.float64_val, 8);
            pos += 8;
            break;
        ...
        case DLMS_DATA_OCTET_STRING: {
            ...
            dst[pos++] = 9;
            size_t lw = 0;
            int ret = encode_length_prefix(value->u.octet_string.len, dst + pos, cap - pos, &lw);
            if (ret != DLMS_OK) return ret;
            pos += lw;
            if (pos + value->u.octet_string.len > cap) return DLMS_ERROR_BUFFER;
            if (value->u.octet_string.data && value->u.octet_string.len > 0)
                memcpy(dst + pos, value->u.octet_string.data, value->u.octet_string.len);
            pos += value->u.octet_string.len;
            break;
        }
        ...
        case DLMS_DATA_VISIBLE_STRING: {
            ...
            dst[pos++] = 10;
            size_t lw = 0;
            int ret = encode_length_prefix(value->u.string_val.len, dst + pos, cap - pos, &lw);
            if (ret != DLMS_OK) return ret;
            pos += lw;
            if (pos + value->u.string_val.len > cap) return DLMS_ERROR_BUFFER;
            if (value->u.string_val.data && value->u.string_val.len > 0)
                memcpy(dst + pos, value->u.string_val.data, value->u.string_val.len);
            pos += value->u.string_val.len;
            break;
        }
        ...
        case DLMS_DATA_BIT_STRING: {
            ...
            dst[pos++] = 4;
            size_t lw = 0;
            int ret = encode_length_prefix(value->u.bit_string.len, dst + pos, cap - pos, &lw);
            if (ret != DLMS_OK) return ret;
            pos += lw;
            if (pos + value->u.bit_string.len > cap) return DLMS_ERROR_BUFFER;
            if (value->u.bit_string.data && value->u.bit_string.len > 0)
                memcpy(dst + pos, value->u.bit_string.data, value->u.bit_string.len);
            pos += value->u.bit_string.len;
            break;
        }
        ...
        case DLMS_DATA_DATETIME: {
            ...
            dst[pos++] = (uint8_t)(v->u.datetime.year >> 8);
            dst[pos++] = (uint8_t)(v->u.datetime.year & 0xFF);
            dst[pos++] = v->u.datetime.month;
            dst[pos++] = v->u.datetime.day;
            dst[pos++] = 0; /* day_of_week: not used in datetime */
            dst[pos++] = v->u.datetime.hour;
            dst[pos++] = v->u.datetime.minute;
            dst[pos++] = v->u.datetime.second;
            dst[pos++] = v->u.datetime.hundredths;
            dst[pos++] = (uint8_t)(v->u.datetime.deviation >> 8);
            dst[pos++] = (uint8_t)(v->u.datetime.deviation & 0xFF);
            dst[pos++] = v->u.datetime.clock_status;
            break;
        }
        ...
        case DLMS_DATA_DATE: {
            ...
            dst[pos++] = 26;
            dst[pos++] = (uint8_t)(value->u.date.year >> 8);
            dst[pos++] = (uint8_t)(value->u.date.year & 0xFF);
            dst[pos++] = value->u.date.month;
            dst[pos++] = value->u.date.day;
            dst[pos++] = value->u.date.day_of_week;
            break;
        }
        ...
        case DLMS_DATA_TIME: {
            if (cap < 5) return DLMS_ERROR_BUFFER;
            dst[pos++] = 27;
            dst[pos++] = value->u.time_val.hour;
            dst[pos++] = value->u.time_val.minute;
            dst[pos++] = value->u.time_val.second;
            dst[pos++] = value->u.time_val.hundredths;
            break;
        }
        ...
        case DLMS_DATA_STRUCTURE: {
            ...
            dst[pos++] = (uint8_t)value->type;
            size_t lw = 0;
            int ret = encode_length_prefix(value->u.array_val.count, dst + pos, cap - pos, &lw);
            if (ret != DLMS_OK) return ret;
            pos += lw;
            for (uint16_t i = 0; i < value->u.array_val.count; i++) {
                size_t iw = 0;
                ret = axdr_encode(&value->u.array_val.items[i], dst + pos, cap - pos, &iw);
                if (ret != DLMS_OK) return ret;
                pos += iw;
            }
            break;
        ...
        case DLMS_DATA_COMPACT_ARRAY: {
            ...
            if (value->u.octet_string.len > 0 && value->u.octet_string.data) {
                size_t lw = 0;
                int ret = encode_length_prefix(value->u.octet_string.len, dst + pos, cap - pos, &lw);
                if (ret != DLMS_OK) return ret;
                pos += lw;
    ...
    *consumed = 0;
    ...
    switch (tag) {
        ...
        case DLMS_DATA_BOOLEAN:
            ...
            out->u.boolean_val = data[pos++] ? true : false;
            *consumed = pos;
            return DLMS_OK;

        case DLMS_DATA_INTEGER:
            if (len < 2) return DLMS_ERROR_BUFFER;
            out->u.int8_val = (int8_t)data[pos++];
            *consumed = pos;
            return DLMS_OK;

        case DLMS_DATA_UNSIGNED_INTEGER:
            if (len < 2) return DLMS_ERROR_BUFFER;
            out->u.uint8_val = data[pos++];
            *consumed = pos;
            return DLMS_OK;
        ...
        case DLMS_DATA_LONG:
            ...
            out->u.int16_val = (int16_t)((uint16_t)data[pos] << 8 | data[pos + 1]);
            pos += 2;
            *consumed = pos;
            return DLMS_OK;
        ...
        case DLMS_DATA_UNSIGNED_LONG:
            if (len < 3) return DLMS_ERROR_BUFFER;
            out->u.uint16_val = (uint16_t)data[pos] << 8 | data[pos + 1];
            pos += 2;
            *consumed = pos;
            return DLMS_OK;
        ...
        case DLMS_DATA_DOUBLE_LONG:
            ...
            out->u.int32_val = (int32_t)(((uint32_t)data[pos] << 24) | ((uint32_t)data[pos+1] << 16) |
        ...
        case DLMS_DATA_DOUBLE_LONG_UNSIGNED:
            if (len < 5) return DLMS_ERROR_BUFFER;
            out->u.uint32_val = ((uint32_t)data[pos] << 24) | ((uint32_t)data[pos+1] << 16) |
                              ((uint32_t)data[pos+2] << 8) | data[pos+3];
            pos += 4;
            *consumed = pos;
            return DLMS_OK;
        ...
        case DLMS_DATA_LONG64:
            ...
            out->u.int64_val = 0;
            for (int i = 0; i < 8; i++)
                out->u.int64_val = (out->u.int64_val << 8) | data[pos++];
            *consumed = pos;
            return DLMS_OK;
        ...
        case DLMS_DATA_UNSIGNED_LONG64:
            ...
            out->u.uint64_val = 0;
            for (int i = 0; i < 8; i++)
                out->u.uint64_val = (out->u.uint64_val << 8) | data[pos++];
            *consumed = pos;
            return DLMS_OK;

        case DLMS_DATA_FLOAT32:
            if (len < 5) return DLMS_ERROR_BUFFER;
            memcpy(&out->u.float32_val, data + pos, 4);
            pos += 4;
            *consumed = pos;
            return DLMS_OK;
        ...
        case DLMS_DATA_FLOAT64:
            if (len < 9) return DLMS_ERROR_BUFFER;
            memcpy(&out->u.float64_val, data + pos, 8);
            pos += 8;
            *consumed = pos;
            return DLMS_OK;
        ...
        case DLMS_DATA_BCD: {
            ...
            if (tag == DLMS_DATA_OCTET_STRING || tag == DLMS_DATA_BIT_STRING || tag == DLMS_DATA_BCD) {
                out->u.octet_string.data = data + pos;
                out->u.octet_string.len = (uint16_t)slen;
            } else {
                out->u.string_val.data = (const char *)(data + pos);
                out->u.string_val.len = (uint16_t)slen;
            }
            pos += slen;
            *consumed = pos;
            return DLMS_OK;
        ...
        case DLMS_DATA_DATETIME:
            ...
            out->u.datetime.year = (uint16_t)((data[pos] << 8) | data[pos + 1]); pos += 2;
            out->u.datetime.month = data[pos++];
            out->u.datetime.day = data[pos++];
            pos++; /* skip day_of_week byte */
            out->u.datetime.hour = data[pos++];
            out->u.datetime.minute = data[pos++];
            out->u.datetime.second = data[pos++];
            out->u.datetime.hundredths = data[pos++];
            out->u.datetime.deviation = (int16_t)((data[pos] << 8) | data[pos + 1]); pos += 2;
            out->u.datetime.clock_status = data[pos++];
            *consumed = pos;
            return DLMS_OK;

        case DLMS_DATA_DATE:
            if (len < 6) return DLMS_ERROR_BUFFER;
            out->u.date.year = (uint16_t)((data[pos] << 8) | data[pos + 1]); pos += 2;
            out->u.date.month = data[pos++];
            out->u.date.day = data[pos++];
            out->u.date.day_of_week = data[pos++];
            *consumed = pos;
            return DLMS_OK;

        case DLMS_DATA_TIME:
            if (len < 5) return DLMS_ERROR_BUFFER;
            out->u.time_val.hour = data[pos++];
            out->u.time_val.minute = data[pos++];
            out->u.time_val.second = data[pos++];
            out->u.time_val.hundredths = data[pos++];
            *consumed = pos;
            return DLMS_OK;
        ...
        case DLMS_DATA_STRUCTURE: {
            ...
            if (ret != DLMS_OK) return ret;
            pos += lc;
            out->u.array_val.count = (uint16_t)count;
            out->u.array_val.items = NULL; /* Caller must manage array storage */
            out->u.array_val.capacity = (uint16_t)count;
            *consumed = pos;
            return DLMS_OK;
        }
